...
5. **Configuration Provisioning/Remediation** - (beta) Is a Nornir process that takes the Intended Configuration, applies the Post-Processing to get a final configuration artifact, and then will push the configuration to the target network device. This push can be a full configuration replace, or a partial remediation per feature, depending on the options selected in the Job.
