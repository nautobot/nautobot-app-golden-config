...
For environments with strict requirements, the recommended approach is to keep using Git but control where it lives: a private, internally hosted repository with role-based access, encryption at rest, audit logging, and no external mirror, combined with thorough `Config Removals` and `Config Replacements` rules for your platforms. This satisfies the large majority of security reviews without working against the app's design.
